...
export default function HomePage({ navigation }) {
  ...
  const [stats, setStats] = useState([
    { label: 'Pedidos', value: '...', icon: 'shopping-bag', color: HEX.info, route: 'SalesOrdersIndex' },
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        setStats([
          { label: 'Pedidos', value: String(ordersRes?.totalItems ?? '—'), icon: 'shopping-bag', color: HEX.info, route: 'SalesOrdersIndex' },
  ...
  return (
    <View style={[styles.container, { backgroundColor: brandColors.background }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        >
          <ShortcutsRow>
            <ShortcutCard label="Pedidos de Venda" icon="shopping-bag" color={HEX.info} onPress={() => go('SalesOrdersIndex')} />
        ...
        <SectionBlock title="PPC e Integrações" icon="monitor" color={HEX.purple} last>
          ...
          <ShortcutsRow>
            <ShortcutCard
              label="Histórico 99Food"
              icon="clock"
              color={HEX.orange}
              onPress={() => navigation.navigate('SalesOrdersIndex', { app: '99Food' })}
            />
